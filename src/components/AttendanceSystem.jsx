...
const AttendanceSystem = () => {
  ...
  // API Configuration - ganti dengan endpoint Anda
  const API_BASE_URL =
  ...
  const handleFaceDetected = useCallback((count, detected, detections) => {
    console.log(`Faces detected: ${count}`);
    console.log("[PARENT] Face detection:", {
      count,
      detected,
      detections: detections?.length,
    });
    console.log(cameraRef.current.captureImage());
  ...
  // API Functions
  const registerFaceToAPI = async (faceData) => {
    try {
  ...
  const verifyFaceWithAPI = async (employeeId, faceImage) => {
    try {
  ...
  const saveAttendanceToAPI = async (attendanceData) => {
    try {
  ...
  const registerFace = async () => {
    ...
    try {
      ...
      // Kirim ke API
      const result = await registerFaceToAPI(faceData);

      // Update state lokal
      setRegisteredFaces((prev) => [...prev, result]);
      ...
      alert("Gagal mendaftarkan wajah ke server. Silakan coba lagi.");
    }
  ...
  const processAttendance = async () => {
    ...
    try {
      ...
      const verificationResult = await verifyFaceWithAPI(
        employee.id,
        faceImage
      );

      if (!verificationResult.success || verificationResult.confidence < 85) {
        alert(
          `Wajah tidak dikenali atau ID tidak cocok (akurasi: ${
            verificationResult.confidence?.toFixed(1) || 0
      ...
      // Update employee data dari hasil verifikasi
      setEmployee((prev) => ({
        ...prev,
        name: verificationResult.employeeName,
      ...
      const attendanceData = {
        employeeId: verificationResult.employeeId,
        ...
        faceImage: faceImage,
        recognitionConfidence: verificationResult.confidence,
      };

      // Simpan ke API
      const saveResult = await saveAttendanceToAPI(attendanceData);
  ...
  useEffect(() => {
    // Load registered faces saat komponen mount
    loadRegisteredFaces();
    fetchDepartments();
  }, []);
  ...
  // Add this state for departments data
