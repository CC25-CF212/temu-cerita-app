...
const AVAILABLE_OFFICE_LOCATIONS = [
  {
    id: "jakarta_pusat",
...
const OFFICE_LOCATION_RADIUS = 2000;
...
const AttendanceSystem = () => {
  ...
  // State untuk status kamera dari CameraComponent
  ...
  useEffect(() => {
    ...
    if (selectedOffice.id === "remote_default") {
      ...
      setCurrentLocation((prev) => ({
        ...prev,
        address: "Mode Lokasi Remote/Manual Terpilih",
      }));
      return;
    }
    ...
    if (
      ...
      if (
        ...
        currentLocation.address !== "Mode Lokasi Remote/Manual Terpilih" &&
        (!currentLocation.address ||
          !currentLocation.address.startsWith("Lat:"))
      ) {
        fetchAddress(currentLocation.latitude, currentLocation.longitude).then(
          (addr) => {
            setCurrentLocation((prev) => ({ ...prev, address: addr }));
          }
        );
      }
    } else {
    ...
    currentLocation.address, // Tambahkan address agar effect bisa re-run jika alamat diubah oleh logic remote
    selectedOffice,
    isLocationLoading,
    calculateDistance,
    locationDisplayStatus, // Dipertahankan agar bisa re-evaluasi alamat jika status berubah
  ]);
  ...
  const validateTime = () => {
    if (!currentTime) return { valid: false, status: "error_waktu" };
